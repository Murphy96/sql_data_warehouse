	...
	BEGIN TRY
		...
		*/
		...
		CASE WHEN TRIM(UPPER(cntry)) IN ('US', 'USA', 'UNITED STATES') THEN 'United States'
			 ...
			 WHEN TRIM(UPPER(cntry)) IN ('AU', 'AUS', 'AUSTRALIA') THEN 'Australia'
			 WHEN TRIM(UPPER(cntry)) IN ('CA','CANADA') THEN 'Canada'
			 WHEN TRIM(UPPER(cntry)) IN ('FR','FRA', 'FRANCE') THEN 'France'
		...
		*/
		...
		PRINT '>> Silver Layer Load Duration: ' + CAST(DATEDIFF(second, @global_start_time, @global_end_time) AS NVARCHAR) + 'seconds';	
